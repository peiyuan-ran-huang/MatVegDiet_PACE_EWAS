...
# This script is to compare EWAS meta-analysis results across different models (for probes available on both 450K and EPIC).
...
veggie_exps <- c("veggie2", "veggie1")
pdi_exps    <- c("PDI", "hPDI", "uPDI")
...
for (main_model in c("AddModel", "FullModel")) {
  ...
  if (length(all_cpgs) == 0L) {
    message("No CpGs found in any exposure – skipping plot.")
    next
  }
  
  for (group in c("veggies", "PDIs")) {
    sub_exps <- if (group == "veggies")
      veggie_exps
    else
      pdi_exps
    sub_cpgs <- all_cpgs[names(all_cpgs) %in% sub_exps]
    
    if (length(sub_cpgs) == 0L) {
      message("No CpG in group ", group, " – skipping.")
      next
    }
    
    plot_data <- rbindlist(lapply(names(sub_cpgs), function(exp) {
      cpgs <- sub_cpgs[[exp]]
      rbindlist(
    ...
    p <- ggforestplot::forestplot(
      ...
      logodds  = FALSE,
      title    = glue(
        "CpGs (available on both 450K and EPIC) with p < 1e-5 in {model_labels[[main_model]]}"
      )
    ) +
    ...
    ggsave(
      filename = file.path(
        plot_dir,
        glue("{prefix}_ModelCompare_{main_model}.{group}.png")
      ),
      plot   = p,
      width  = ifelse(group == "veggies", 8, 14),
      height = 4 * length(unique(plot_data$Exposure)),
      dpi    = 300
    )
    
    message("→ Forest plot saved: ", main_model, ".", group)
  }
}
