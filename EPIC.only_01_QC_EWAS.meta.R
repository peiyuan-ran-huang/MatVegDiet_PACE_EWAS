...
# This script is to perform QC for EWAS meta-analysis (for probes unique to EPIC).
...
for (model in model_list) {
  ...
  for (exp in names(meta_dt)) {
    ...
    abline(h = suggest_y, lty = 2)
    abline(h = bonf_y, lty = 3)
    dev.off()
...
message("QC pipeline finished successfully for all models on EPIC-only data!")
...
combined <- ggdraw() +
  draw_label(
    "EWAS meta-analysis (probes unique to EPIC)",
    fontface = "bold",
    x        = 0.01,
    y        = 0.98,
    hjust    = 0,
